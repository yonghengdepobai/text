...
(function () {
    ...
    SimpleWebGL = function (canvas, config) {
        // this 为SimpleWebGL的实例
        var webgl = this._ = canvas.getContext("experimental-webgl", config); // 获取webgl绘图上下文
        this.setting = function (map, value) { // {DEPTH_TEST:"LESS"} 
            // map = {DEPTH_TEST:"LESS"}; map[i] = 'LESS'
            if (value !== undefined) {// 处理第三个参数
                arguments[2] = {}; arguments[2][map] = value; map = arguments[2]
            };
            for (var i in map) {
                webgl[map[i] === null ? "disable" : "enable"](webgl[i]);  // webgl.enable(DEPTH_TEST)是否开启消除隐藏面功能
                if (!(map[i] instanceof Array)) { map[i] = [map[i]] }; // map[i] = ['LESS']
                for (var j = 0; j < map[i].length; j++) { map[i][j] = webgl[map[i][j]] };
                webgl[{ DEPTH_TEST: "depthFunc", BLEND: "blendFunc" }[i]].apply(webgl, map[i]);
            };
             return this; // 链式返回
        };
        this.clear = function () {// 清除渲染区（比如颜色渲染，深度渲染）
            for (var v = 0, n, i = 0; n = arguments[i]; i++) {
                v |= webgl[n + "_BUFFER_BIT"]
            }
            return v && webgl.clear(v), this; // 链式返回
        };
        this.color = function (e) {
            return webgl.clearColor.apply(webgl, e instanceof Array ? e : arguments), this; // 链式返回
        }
         ...
         this.namespace = function (callback) {
            ...
            for (i = 0; i < e.length; i++){
                s.push(this.namespace[e[i].replace(/\W+/g, "")])
            }
